...
本戦の匿名化プログラム. 使い方はサンプル匿名化プログラムと同じ.
...
### calc_cramersV.ipynb で算出
swap_columns = [
    ## 0
    [['Occupation', 'Gender', '3877'], ['2193', '1525', '3440'], ['2872', '2253', '2628'], ['ZIP-code']],
    ## 1
    [['Occupation', 'Gender', '1967'], ['1525', '2628', '2043'], ['2143', '2399', '1750'], ['2093', '2193', '2968'], ['ZIP-code']],
    ## 2
    [['Occupation', '2087'], ['ZIP-code']],
    ## 3
    [['Age', 'Gender', '1073'], ['1967', '2087', '56'], ['2017', '1525', '1920'], ['ZIP-code']],
    ## 4
    [['Occupation', 'Gender', '3466'], ['ZIP-code']],
    ## 5
    [['Occupation'] , ['1654', '2086'], ['ZIP-code']],
    ## 6
    [['Gender', '2143', '2872'], ['ZIP-code']],
    ## 7
    [['Occupation', '2093'], ['Gender', '1750', '1097'], ['ZIP-code']],
    ## 8
    [['Gender', '1920'], ['ZIP-code']],
    ## 9
    [['Occupation'], ['Gender', '1097', '2193'], ['ZIP-code']]
]
...
pair_swap_columns = [
    ...
    [['1525', '3889']],
    ## 1
    [['673', 'ZIP-code'], ['1525', '2093', '3889'], ['2193', '3479']],
    ## 2
    [],
    ## 3
    [['2017', '2399'], ['673', 'ZIP-code']],
    ## 4
    [['1702', '3466']],
    ## 5
    [],
...
def anonymize(df, subset_id):
    ...
    len_split_df = int(len(anonymized_df) / (num_split + zip_ratio[subset_id] - 1)) # ZIP-code のみ zip_ratio[subset_id] 倍のデータ
    anonymized_df_list = []
...
def group_shuffle(df, groups, targets):
    ...
    for name, group_data in anonymized_df.groupby(groups):
        if len(group_data) == 1:
            ...
            for target in targets:
                for pair_swap_column in pair_swap_columns[subset_id]:
                    if target in pair_swap_column:
                        # Create a temporary DataFrame with the target columns
                        temp_df = group_data[pair_swap_column]
                        # Shuffle the rows of the temporary DataFrame
                        shuffled_df = temp_df.sample(frac=1)
                        # Replace the values in the original DataFrame with the shuffled values
                        for target in pair_swap_column:
                            anonymized_df.loc[group_data.index, target] = shuffled_df[target].values
                        break
                else:
