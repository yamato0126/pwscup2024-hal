...
列ごとの最頻値で埋めるシンプルな攻撃プログラム. 使い方はサンプル攻撃プログラムと同じ.
...
def main(a_prefix, b_prefix, c_prefix):
    ...
    # b.csv の各行について、Tの最頻値で"*"を埋める
    for b_index, b_row in b.iterrows():
        filled_value, column = fill_placeholders(b_row, T_mode, b_row.index)
        filled_values.append(filled_value)
        columns.append(column)
    
    for i in range(len(filled_values)):
        b.at[i, columns[i]] = filled_values[i]
